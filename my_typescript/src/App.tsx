...
export default function App() {
  ...
  const exportXml = () => {
    try {
      ...
      a.href = url;
      a.download = "work_projects.xml";
      a.click();
      URL.revokeObjectURL(url);
      showToast("已下载 work_projects.xml");
    } catch (err) {
        ...
        <section className="detail-panel">
          ...
          <div className="detail-scroll">
            <p className="toolbar-muted" style={{ marginTop: 0 }}>
              与 SolidWorks 插件共用数据：将导出的 work_projects.xml 放到{" "}
              <code>%LOCALAPPDATA%\{PLUGIN_XML_RELATIVE}</code>（可先备份原文件）。
              「复制绑定 JSON」对应插件的 shared_work_project.json。
            </p>
            {CATEGORIES.map((cat) => {
                ...
                <div key={cat.key} className="category-block">
                  ...
                  <div className="field-grid">
                    ...
                    <input
                      ...
                      value={pathStr}
                      onChange={(e) =>
                        current && patchCurrent({ [pathKey]: e.target.value } as Partial<WorkProjectItem>)
                      }
                    ...
                    <textarea
                      ...
                      value={followStr}
                      onChange={(e) =>
                        current && patchCurrent({ [followKey]: e.target.value } as Partial<WorkProjectItem>)
                      }
                    />
                  </div>
                </div>
